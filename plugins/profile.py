...
async def _(ult):
    ...
    try:
        await Asteroid_bot(UpdateProfileRequest(about=set))
...
async def _(ult):
    ...
    photo = None
    try:
        photo = await Asteroid_bot.download_media(reply_message, TMP_DOWNLOAD_DIRECTORY)
    except Exception as ex:
        await ok.edit("Error occured.\n`{}`".format(str(ex)))
    else:
        if photo:
            await ok.edit("`Uploading it to my profile...`")
            file = await Asteroid_bot.upload_file(photo)
            try:
                await Asteroid_bot(UploadProfilePhotoRequest(file))
            except Exception as ex:
                await ok.edit("Error occured.\n`{}`".format(str(ex)))
            else:
...
async def remove_profilepic(delpfp):
    ...
    await Asteroid_bot(DeletePhotosRequest(id=input_photos))
    await ok.edit(f"`Successfully deleted {len(input_photos)} profile picture(s).`")
    await asyncio.sleep(10)
    await ok.delete()


@Asteroid_cmd(pattern="poto ?(.*)")
async def gpoto(e):
    ...
    if not (ult or e.is_reply):
        ult = e.chat_id
    try:
        okla = await Asteroid_bot.download_profile_photo(
            ult,
            "profile.jpg",
            download_big=True,
        )
        await a.delete()
        await Asteroid_bot.send_message(e.chat_id, file=okla)
        os.remove(okla)
