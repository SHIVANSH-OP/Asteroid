...
async def waifu(animu):
    ...
    try:
        sticcers = await Asteroid_bot.inline_query(
            "stickerizerbot",
...
async def hehe(args):
    ...
    user = await Asteroid_bot.get_me()
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            photo = io.BytesIO()
            photo = await Asteroid_bot.download_media(message.photo, photo)
        elif "image" in message.media.document.mime_type.split("/"):
            await xx.edit(f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            await Asteroid_bot.download_file(message.media.document, photo)
            if (
            ...
            await xx.edit(f"`{random.choice(KANGING_STR)}`")
            await Asteroid_bot.download_file(
    ...
    if photo:
        ...
        pack = 1
        if len(splat) == 3:
            pack = splat[2]  # User sent Asteroid_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with Asteroid_bot.conversation("@Stickers") as conv:
                await conv.send_message("/addsticker")
                await conv.get_response()
                await Asteroid_bot.send_read_acknowledge(conv.chat_id)
...
async def ultdestroy(event):
    ...
    if not (
        ...
        return await eor(event, "`Reply to Animated Sticker only`")
    await event.client.download_media(ult, "Asteroid.tgs")
    xx = await eor(event, "`Processing...`")
    os.system("lottie_convert.py Asteroid.tgs json.json")
    json = open("json.json")
    jsn = json.read()
    json.close()
    jsn = (
    ...
    os.system("lottie_convert.py json.json Asteroid.tgs")
    await event.client.send_file(
        event.chat_id,
        file="Asteroid.tgs",
        force_document=False,
...
async def ultiny(event):
    ...
    if not (reply and (reply.media)):
        ...
        return
    xx = await eor(event, "`processing...`")
    ik = await Asteroid_bot.download_media(reply)
    im1 = Image.open("resources/extras/Asteroid_blank.png")
    if ik.endswith(".tgs"):
