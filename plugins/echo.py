...
from pyAsteroid.functions.echo_db import *
from telethon.utils import get_display_name

from . import *


@Asteroid_cmd(pattern="addecho ?(.*)")
async def echo(e):
...
async def okk(e):
    if check_echo(e.chat_id, e.sender_id):
        try:
            ...
            return await Asteroid_bot.send_message(e.chat_id, ok)
        except Exception as er:
            LOGS.info(er)


@Asteroid_cmd(pattern="listecho$")
async def lstecho(e):
    k = list_echo(e.chat_id)
    if k:
        user = "**Activated Echo For Users:**\n\n"
        for x in k:
            ok = await Asteroid_bot.get_entity(int(x))
