...
@Asteroid_cmd(pattern="kickme$", groups_only=True, allow_sudo=False)
async def leave(ult):
    await eor(ult, f"`{Asteroid_bot.me.first_name} has left this group, bye!!.`")
    await Asteroid_bot(LeaveChannelRequest(ult.chat_id))
...
async def _(event):
    ...
    result = await Asteroid_bot(GetAdminedPublicChannelsRequest())
    output_str = ""
    r = result.chats
...
async def stats(
    ...
    unread = 0
    dialog: Dialog
    async for dialog in Asteroid_bot.iter_dialogs():
    ...
    ct = (await Asteroid_bot(GetBlockedRequest(1, 0))).count
    full_name = inline_mention(await Asteroid_bot.get_me())
...
async def _(event):
    ...
    try:
        ok = await Asteroid_bot.inline_query(asst.me.username, q)
...
async def rmbg(event):
    ...
    if event.reply_to_msg_id:
        ...
        dl = await Asteroid_bot.download_media(reply.media)
    ...
    await Asteroid_bot.send_file(event.chat_id, "ult.webp", reply_to=reply)
    os.remove(rmbgp)
    os.remove("ult.webp")
    await xx.delete()
...
async def telegraphcmd(event):
    ...
    if event.reply_to_msg_id:
        ...
        if getmsg.photo or getmsg.video or getmsg.gif:
            getit = await Asteroid_bot.download_media(getmsg)
            try:
                variable = uf(getit)
                os.remove(getit)
