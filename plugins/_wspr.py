...
async def _(e):
    if e.reply_to_msg_id:
        ...
        try:
            zyx = await Asteriod_bot(gu(id=okk))
    ...
    if put:
        try:
            results = await Asteriod_bot.inline_query(asst.me.username, f"msg {put}")
        except rep:
            return await eor(
                e,
...
async def _(e):
    ...
    if "wspr" not in vvv:
        try:
            logi = await Asteriod_bot(gu(id=query))
        ...
        try:
            logi = await Asteriod_bot.get_entity(query)
            button = [
